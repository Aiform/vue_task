		...
		<el-table style="width: 100%" :data="tabledata">
			...
			<el-table-column fixed="right" label="操作" width="200">
				<template #default="scope">
					<el-button link type="primary" size="small" @click="priew(scope.$index,scope.row)" >预览</el-button>
					<el-button link type="primary" size="small" @click="relieve(scope.$index,scope.row)">解除</el-button>
					<el-button link type="primary" size="small" @click="updatedata(scope.$index,scope.row)">修改</el-button>
					<el-button link type="primary" size="small" @click="deleteinfor(scope.$index,scope.row)">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<el-dialog v-model="dialogTableVisible" :show-close="false"  v-for="(form,index) in formdata" :key="form.id" id="printContainer">
	...
	export default{
		...
		setup(){
			...
			const previewofisolationpersonnel = (row)=>{
				axios.post('/previewofisolationpersonnel',qs.stringify({id:row})).then((result)=>{
			...
			const preservation = ()=>{
				formdata.value.map((value,index,array)=>{
					if(!textempty.test(value.name,value.address,value.Isolationaddress)){
			...
			const deleteinfor = (index,row)=>{
				...
				.then(() => {
					...
					axios.post('/isolationpersonneldeletionfunction',qs.stringify({id:row.id})).then((result)=>{
						if(result.data.status == 0){
							myFn()
						}
					})
