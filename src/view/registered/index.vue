<template>
	<div class="voidsbp_targ_dealloc_login">
		<el-card class="box-card">
		  		<div class="header">
		  			<el-icon style="font-size:25px; margin-right: 5px"><Avatar /></el-icon><h3>疫情人员信息与数据后台管理系统注册页面</h3>
		  		</div>
		  		<el-divider />
		  		 <div class="infor">
			      	<div class="inforAvatr">
			      		<el-form  class="demo-form-inline">
						    ...
						    <el-form-item label="注册账号:">
						      <el-input v-model.trim="infor.username" placeholder="请输入注册账号"  />
						      <p>最少5位，最多10位,包括至少个大小写字母数字</p>
						    </el-form-item>
						     <el-form-item label="注册密码:">
						      <el-input v-model.trim="infor.password" type="password" show-password placeholder="请输入注册密码"  />
						       <p>最少5位,最多10位,包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符。</p>
						    </el-form-item>

						    <el-form-item label="确认密码:">
						      <el-input  v-model.trim="infor.repassword" type="password" show-password placeholder="请输入确认密码"/>
						       <p>最少5位,最多10位,包括至少1个大写字母，1个小写字母，1个数字，1个特殊字符。</p>
						    </el-form-item>
						     ...
						     <el-form-item label="邮箱地址:">
						      <el-input v-model.trim="infor.email" type="email" placeholder="请输入邮箱地址"  />
						    </el-form-item>
						    <el-form-item label="真实姓名:">
						      <el-input v-model.trim="infor.name"  placeholder="请输入姓名"  />
						    </el-form-item>
						    <el-form-item label="家庭地址:">
						      <el-input v-model.trim="infor.address" placeholder="请输入家庭地址"  />
						    </el-form-item>
						  ...
						  <div class="Avatar">
						  		<div class="AvataruserPic">
						  				<img src="http://localhost:3008/uploads/2022.png" width="70" height="70" style="border-radius: 50%;">
						  		</div>
						  		 <el-upload
							    v-model="fileList"
							    class="upload-demo"
...
import {userPic,adminName,Phone,Email,Name,Role,Address,Sex,Remarks,Id} from '@/token'
export default{
	name:'registered',
	setup(){
		const fileList = ref('')
		...
		const repassword = /^.*(?=.{5,10})(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*? ]).*$/
		// 手机号的验证规则
		const phone = /^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\d{8}$/
		// 邮箱的验证规则
		const email = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,6})$/
		const registered = ()=>{
			if(infor.adminname == '' || infor.role == '' || infor.sex == '' || infor.name == '' || infor.address == '' || infor.remarks == '' ){
