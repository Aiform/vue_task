      ...
      <div class="newlyAdded">
        <el-form style="display: flex; flex-wrap: wrap">
          ...
          <div class="but">
            ...
            <el-button v-if="endqueryIsshow" @click="endquery"
              >结束查询</el-button
            >
          </div>
        </el-form>
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="scope">
            ...
            <el-button
              ...
              type="primary"
              size="small"
              @click="medicalstaffmodificationfunction(scope.$index, scope.row)"
              >修改</el-button
            >
            <el-button
              link
...
export default {
  ...
  setup() {
    ...
    const handleCurrentChange = (num) => {
      axios
        .post("/protectivepersonnel", qs.stringify({ num: num }))
        .then((result) => {
      ...
      if (keyword.value == "") {
        ...
        setTimeout(() => {
          ...
          axios
            .post(
              "/protectivepersonnel",
              qs.stringify({ num: num, keyword: keyword.value })
            )
            .then((result) => {
              if (result.data.status == 0) {
                tabledata.value = result.data.data;
    ...
    const preview = (index, row) => {
      axios
        .post("/previewfunctionofmedicalstaff", qs.stringify({ id: row.id }))
        .then((result) => {
    ...
    const medicalstaffmodificationfunction = (index, row) => {
      id.value = row.id;
      axios
        .post("/previewfunctionofmedicalstaff", qs.stringify({ id: id.value }))
        .then((result) => {
          previewdata.value = result.data.data;
        });
      const loading = ElLoading.service({
