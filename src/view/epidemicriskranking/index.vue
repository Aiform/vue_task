  ...
  <div class="epidemicriskranking">
    <el-card>
      <div class="header">
        <div class="Promotion">
          <h4>
            <el-icon style="color: #10aeb5; font-size: 20px"
              ><Promotion /></el-icon
            >疫情速报
          </h4>
          <div style="display: flex">
            <el-form-item label-width="120px" style="margin-right: 10px">
              <el-input
                ...
                @change="serch"
                placeholder="请输入你要搜索的地区"
              />
            </el-form-item>
            <el-button
              style="background: #10aeb5; color: #fff"
              @click="newaddress"
        ...
        <el-alert
          show-icon
          title="该数据为31省（自治区、直辖市）本土新增，及港澳台新增确诊数据。每天上午7：00-10：00更新数据"
          type="warning"
        />
      </div>
      <div class="tableData">
      ...
      <el-alert
        show-icon
        title="数据分析统计图，以下数据会根据上面修改的数据，自动更新"
        type="warning"
        class="warning"
      />
      ...
      <el-dialog v-model="visible" :show-close="false">
        ...
        <el-form
          label-width="120px"
          v-for="(item, index) in handleEditArray"
          :key="item.id"
        >
        ...
        <div class="but">
          <el-button
            ...
            >确定更新</el-button
          >
          <el-button style="background: #10aeb5; color: #fff" @click="esc"
            >退出更新</el-button
          >
        </div>
      </el-dialog>
...
export default {
  ...
  setup() {
    ...
    const ModifHaleEdit = (row) => {
      axios
        .post("/updateofepidemicareadataprev", qs.stringify({ id: row }))
        .then((result) => {
    ...
    const handleDelete = (index, row) => {
      ElMessageBox.confirm("此操作将永久删除", "警告", {
        ...
        .then(() => {
          ...
          axios
            .post("/datadeletioninepidemicareas", qs.stringify({ id: row.id }))
            .then((result) => {
              if (result.data.status == 0) {
                myFn();
              }
