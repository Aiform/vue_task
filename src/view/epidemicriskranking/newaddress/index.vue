    ...
    <el-card>
      ...
      <el-form label-width="120px">
        <el-form-item label="城市:">
          <el-input
            placeholder="请输入省会与市区--例如:郑州（河南省）只能输入省会城市"
            @focus="focus"
            @change="change"
            v-model.trim="address.provincial"
          />
        </el-form-item>
        ...
        <el-form-item label="新增本地:">
          <el-input
            placeholder="请输入新增本地数据"
            v-model.trim.Number="address.localdata"
          />
        </el-form-item>
        <el-form-item label="新增无症状:">
          <el-input
            placeholder="请输入新增无症状数据"
            v-model.trim.Number="address.asymptomaticdata"
          />
        </el-form-item>
        <el-form-item label="风险区域:">
          <el-input
            placeholder="请输入风险区域数据"
            v-model.trim.Number="address.risk"
          />
        </el-form-item>
        <div class="but">
  ...
  setup() {
    ...
    const change = () => {
      if (address.provincial == "") {
        ...
        axios
          .post(
            "/epidemicriskprompt",
            qs.stringify({ provincial: address.provincial })
          )
          .then((result) => {
    ...
    const ConfirmAddition = () => {
      if (!textempty.test(address.provincial)) {
        ...
        !sum.test(address.localdata, address.asymptomaticdata, address.risk)
      ) {
