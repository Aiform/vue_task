        ...
        <el-table :data="filterTableData" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope">
              <el-button circle @click="preview(scope.$index, scope.row)"
      ...
      >
        ...
        <template #footer>
          <span class="dialog-footer">
            ...
            <el-button v-if="determineShow" @click="determine"
              >保存修改</el-button
            >
  ...
  setup() {
      ...
      if (presen.keyword == "") {
        ...
        setTimeout(() => {
          ...
          axios
            .post(
              "/pading",
              qs.stringify({ num: num, keyword: presen.keyword })
            )
            .then((result) => {
    ...
    const determine = () => {
      tabledata.value.map((value) => {
        if (
          !textempty.test(value.name, value.race, value.address, value.work)
        ) {
          ...
          !word.test(value.sally, value.cold, value.report, value.sex)
        ) {
